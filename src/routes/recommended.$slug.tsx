...
type Site = {
  slug: string; name: string; location: string; rating: number; tag: string;
  description: string; amenities: string[]; photos: string[];
  review_author: string; review_text: string;
};
...
function SiteDetail() {
  ...
  useEffect(() => {
    supabase.from("recommended_sites").select("*").eq("slug", slug).maybeSingle().then(({ data }) => {
  ...
  return (
    <article className="mx-auto max-w-6xl px-4 py-10 sm:py-14">
      ...
      <p className="mt-6 max-w-3xl text-lg text-foreground/85">{site.description}</p>
